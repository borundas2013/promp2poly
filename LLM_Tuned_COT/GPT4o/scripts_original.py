...
def prepare_prompt(monomer1, monomer2, er, tg):
    ...
    return user_prompt_list, assistant_prompt_list,system_prompt
...
def save_polymer_conversation(user_prompt_list, assistant_prompt_list, system_prompts):
    ...
    output_file_train = os.path.join("LLM_Tuned_COT", "Data", "training_conversations_large.jsonl")
    output_file_valid = os.path.join("LLM_Tuned_COT", "Data", "validation_conversations_large.jsonl")


    split_idx = int(len(user_prompt_list) * 0.8)
    ...
    for i in range(len(train_user_data)):
        conversation = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": train_system[i]
                },
                {
                    "role": "user",
                    "content": train_user_data[i]
                },
                {
                    "role": "assistant", 
                    "content": train_assistant[i]
                }
            ]
        }
