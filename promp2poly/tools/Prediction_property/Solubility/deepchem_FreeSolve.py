...
def train_and_evaluate_model():
    ...
    # Load the dataset
    featurizer = dc.feat.MorganFingerprint(size=1024, radius=2)
    tasks, datasets, transformers = dc.molnet.load_freesolv(featurizer='ECFP', split='random')
...
def save_model_and_featurizer(model, featurizer, tasks, best_epoch, best_valid_mse):
    os.makedirs('Prediction_property/Solubility/saved_models/saved_models', exist_ok=True)

    # Save model weights
    model.save_checkpoint(model_dir='Prediction_property/Solubility/saved_models')
    ...
    with open('Prediction_property/Solubility/saved_models/model_info.pkl', 'wb') as f:
...
def predict_property_combined(model, featurizer, tasks, smiles_combined: str):
    ...
    return {
        ...
        "predicted_hydration_free_energy": float(prediction[0][0]),  # kcal/mol
        "property": "Hydration Free Energy",
        "units": "kcal/mol"
    }
...
if __name__ == "__main__":
    ...
    if os.path.exists('saved_models/best_hydration_model'):
