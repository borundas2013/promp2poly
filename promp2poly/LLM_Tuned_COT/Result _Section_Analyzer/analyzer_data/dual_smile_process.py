...
def encode_functional_groups(monomer1_list, monomer2_list):
    ...
    for m1, m2 in zip(monomer1_list, monomer2_list):
        ...
        if count_functional_groups(m1, constants.BEZEN_SMARTS) >= 2:
            found_groups_m1.append("C1OC1")
        if count_functional_groups(m1, constants.EMBEDDING_DIMARTS) >= 2:
        ...
        if not (count_functional_groups(m1, constants.BEZEN_SMARTS) >= 2 or 
                count_functional_groups(m1, constants.EMBEDDING_DIMARTS) >= 2 or
        ...
        if count_functional_groups(m2, constants.BEZEN_SMARTS) >= 2:
            found_groups_m2.append("C1OC1")
        if count_functional_groups(m2, constants.EMBEDDING_DIMARTS) >= 2:
        ...
        if not (count_functional_groups(m2, constants.BEZEN_SMARTS) >= 2 or 
                count_functional_groups(m2, constants.EMBEDDING_DIMARTS) >= 2 or
...
def check_reaction_validity_with_invalid_groups(smiles1, smiles2):
    ...
    pairs = [
        ...
        (constants.BEZEN_SMARTS, constants.EMBEDDING_DIMARTS, ['C1OC1', 'NC']),
        (constants.EMBEDDING_DIMARTS, constants.BEZEN_SMARTS, ['NC', 'C1OC1']),
...
def check_reaction_validity(smiles1, smiles2):
    ...
    if mol1 is None or mol2 is None:
        return False,[]
    if count_functional_groups(smiles1, constants.BEZEN_SMARTS) >= 2 and count_functional_groups(smiles2, constants.EMBEDDING_DIMARTS) >= 2:
        return True,['C1OC1','NC']
    if count_functional_groups(smiles1, constants.EMBEDDING_DIMARTS) >= 2 and count_functional_groups(smiles2, constants.BEZEN_SMARTS) >= 2:
